...
The table is tidy cause each row contains a distinct observation about a specific unit of a nuclear power plant and each column represents a single variable such as 'Plant Name'. For power plants with multiple units, the Plant name is repeated in multiple rows this can be circumvented. In addition to this, there seems to be a lot of plain missing data. Also, the dates need to be converted to numeric format and the missing data needs to be filled in with N/A which is reccurent for currently operational plants.Lastly, the superscripts attribute to possible errors during data visualization since they make for new categories in certain variables for eg: i) Operational ii) Operational[81]. There are ways to make this table less redundant .
...
```{r}
japan <- japan %>%
  mutate(reactor = as.character(reactor))
...
japan_data <- combined_data %>%
  ...
  mutate(
    duration = as.numeric(difftime(operation, construction, units = "days"))
  )
...
ggplot(japan_data, aes(y = reorder(name, construction), x = construction, xend = operation, color = status, linetype = type)) +
  geom_segment(aes(xend = operation), size = 1.2) +
  labs(
    title = "Info Graphic regarding Commissioning for Nuclear Reactors in Japan",
    x = "Date",
    y = "Reactor Name",
    color = "Status",
    linetype = "Type"
